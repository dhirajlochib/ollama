...
func displayDiffusionProgress(block, step, maxSteps, unmasked, blockSize int) {
	...
	barWidth := 20
	filled := int(float64(unmasked) / float64(blockSize) * float64(barWidth))
	if filled > barWidth {
		filled = barWidth
	}
	...
	fmt.Printf("\r\x1b[K\x1b[2;36m⟨Diffusion Block %d⟩\x1b[0m %s \x1b[33mstep %d/%d\x1b[0m \x1b[32m(%d/%d tokens)\x1b[0m",
		block, bar, step, maxSteps, unmasked, blockSize)
}
	...
	fn := func(response api.ChatResponse) error {
		...
		if response.DiffusionBlockSize > 0 {
			...
			displayDiffusionProgress(
				...
				response.DiffusionMaxSteps,
				response.DiffusionUnmasked,
				response.DiffusionBlockSize,
			)
			// When a block is fully unmasked, print the finalized tokens
			if response.DiffusionUnmasked >= response.DiffusionBlockSize {
				if response.DiffusionBlock != lastDiffBlock {
					fmt.Println() // finish progress line
					lastDiffBlock = response.DiffusionBlock
				}
			}
			// The actual content still comes through Message.Content
		}
