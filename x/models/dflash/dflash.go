...
func (a *Attention) Forward(x *mlx.Array, b *batch.Batch, c cache.Cache, positions *mlx.Array, B, L int32, cfg *Config) *mlx.Array {
	...
	k, v := propK, propV
	// KVCache / RotatingKVCache expose View for read-only history; assert
	// locally so we don't require a named Viewer interface on cache.Cache.
	if viewer, ok := c.(interface {
		View(*batch.Batch) *nn.KVHistory
	}); ok {
